...
BHV_AutoWaypoint::BHV_AutoWaypoint(IvPDomain gdomain) : 
  IvPBehavior(gdomain)
{
  // First set variables at the superclass level
  m_descriptor = "BHV_AutoWaypoint";  
...
void BHV_AutoWaypoint::onSetParamComplete()
{
...
bool BHV_AutoWaypoint::setParam(string param, string param_val) 
{
...
void BHV_AutoWaypoint::onIdleToRunState() 
{
  if(!updateInfoIn()) 
    return;
  m_odo_leg_disq = true;
...
void BHV_AutoWaypoint::onRunToIdleState() 
{
  postErasables();
...
BehaviorReport BHV_AutoWaypoint::onRunState(string input) 
{
...
IvPFunction *BHV_AutoWaypoint::onRunState() 
{
...
void BHV_AutoWaypoint::onIdleState() 
{
  if(!updateInfoIn()) 
    return;
}
...
bool BHV_AutoWaypoint::updateInfoIn()
{
...
bool BHV_AutoWaypoint::setNextWaypoint()
{
  ...
  if(m_lead_distance >= 0) {
    ...
    if(track_anchor) {
      ...
      if(disp_error) {
        total_error += dist_to_trackline;
        postMessage("DIST_TRACKLINE",total_error);
      }
      if(m_lead_damper > 0) {
        //double dist_to_trackline = hypot((nx-m_osx),(ny-m_osy));
...
IvPFunction *BHV_AutoWaypoint::buildOF(string method) 
{
  IvPFunction *ipf = 0;
...
void BHV_AutoWaypoint::postStatusReport()
{
  int    current_waypt = m_waypoint_engine.getCurrIndex();
...
void BHV_AutoWaypoint::postViewableSegList()
{
...
void BHV_AutoWaypoint::postErasables()
{
  postMessage("VIEW_POINT", m_trackpt.get_spec("active=false"), "trk");
  postMessage("VIEW_POINT", m_nextpt.get_spec("active=false"), "wpt");
...
void BHV_AutoWaypoint::postCycleFlags()
{
...
void BHV_AutoWaypoint::postWptFlags(double x, double y)
{
...
void BHV_AutoWaypoint::handleVisualHint(string hint)
{
...
void BHV_AutoWaypoint::updateOdoDistance()
{
  if(!m_odo_virgin) {
...
void BHV_AutoWaypoint::markOdoLeg()
{
