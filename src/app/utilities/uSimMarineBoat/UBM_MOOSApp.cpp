...
bool UBM_MOOSApp::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool UBM_MOOSApp::OnStartUp()
{
  ...
  for(p = sParams.begin();p!=sParams.end();p++) {
    ...
    else if((param == "FRICTION_COEF") && isNumber(value))
      handled = m_model.setParam("friction_coef", dval);
  ...
  if(!m_MissionReader.GetValue("LatOrigin", latOrigin)) {
    MOOSTrace("uSimMarineBoat: LatOrigin not set in *.moos file.\n");
    m_geo_ok = false;
  } 
  else if(!m_MissionReader.GetValue("LongOrigin", longOrigin)) {
    MOOSTrace("uSimMarineBoat: LongOrigin not set in *.moos file\n");
    m_geo_ok = false;      
  }
  else {
    m_geo_ok = true;
    ...
    if(!m_geodesy.Initialise(latOrigin, longOrigin)) {
      MOOSTrace("uSimMarineBoat: Geodesy init failed.\n");
      m_geo_ok = false;
    }
  }
  ...
  registerVariables();
  MOOSTrace("uSimMarineBoat started \n");
  return(true);
}
...
bool UBM_MOOSApp::OnConnectToServer()
{
  registerVariables();
...
void UBM_MOOSApp::cacheStartingInfo()
{
...
void UBM_MOOSApp::registerVariables()
{
...
void UBM_MOOSApp::postNodeRecordUpdate(string prefix,
...
bool UBM_MOOSApp::handleThrustMapping(string mapping)
{
  if(mapping == "")
    return(false);
...
string UBM_MOOSApp::handleConfigDeprecations(string param)
{
