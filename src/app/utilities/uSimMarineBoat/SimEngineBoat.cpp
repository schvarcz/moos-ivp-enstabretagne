...
void SimEngineBoat::propagate(NodeRecord &record, double delta_time,
			  double prior_heading, double prior_speed,
                          double drift_x, double drift_y,double rudder)
{
  ...
  double xdot  = (sin_ang * speed * cos(degToRadians(rudder)));
  double ydot  = (cos_ang * speed * cos(degToRadians(rudder)));
...
void SimEngineBoat::propagateDepth(NodeRecord& record,
			       double delta_time,
...
void SimEngineBoat::propagateSpeed(NodeRecord& record, const ThrustMap& tmap,
			       double delta_time, double thrust,
			       double rudder, double max_accel, 
                               double max_decel,double friction)
{
  if(delta_time <= 0)
    return;
...
//  double next_speed  = tmap.getSpeedValue(thrust);
  double next_speed  = thrust*delta_time;
...
//  }
  next_speed -= prev_speed*friction;
...
void SimEngineBoat::propagateHeading(NodeRecord& record,
				 double delta_time, 
				 double rudder,
				 double thrust,
                                 double turn_rate)
...
//  delta_deg += (delta_time * rotate_speed);
  double delta_deg = speed*sin(degToRadians(rudder))*delta_time*turn_rate;
  ...
  //new_heading = new_heading>max_angle?max_angle:new_heading;
  //new_heading = new_heading<-max_angle?-max_angle:new_heading;
...
void SimEngineBoat::propagateSpeedDiffMode(NodeRecord& record, const ThrustMap& tmap,
				       double delta_time, 
				       double thrust_lft,
				       double thrust_rgt, 
				       double max_accel, 
                                       ...
                                       double friction)
{
  ...
  propagateSpeed(record, tmap, delta_time, thrust, rudder, max_accel, max_decel,friction);
}
...
void SimEngineBoat::propagateHeadingDiffMode(NodeRecord& record,
					 double delta_time, 
					 double thrust_lft,
					 double thrust_rgt,
					 double turn_rate,
